...
// Create UWB instance (adjust pins as needed)
...
void setup() {
    ...
    if (!uwb.begin()) {
        Serial.println("ERROR: Failed to init UWB module");
        while(1);
    }
    ...
    Serial.println("Storing small message periodically for ANCHOR to request");
}
...
void loop() {
    ...
    if (now - lastStore >= STORE_INTERVAL) {
        ...
        Serial.print("Storing message in TAG: "); Serial.println(payload);
        bool ok = uwb.sendMessageFromTag(payload);
        if (!ok) {
            Serial.println("Failed to store message in TAG");
        }
    }
