/*
  ESP32-C3 Anchor Example (Async / Sync)
  Converted from: base-lib-realtime-arduino/src/esp32c3/main.cpp

  This example demonstrates two methods to poll a TAG from an ANCHOR:
  - Asynchronous (recommended): non-blocking, uses callback on response
  - Synchronous (blocking): waits for a reply (simpler but blocks loop)

  Wiring:
   - Connect RYUW122 TX -> ESP32 RX (pin defined below)
   - Connect RYUW122 RX -> ESP32 TX (pin defined below)
   - Connect RYUW122 NRST -> RESET_PIN (active LOW)

  Notes:
   - Adjust RX_PIN/TX_PIN to match your board wiring.
   - This sketch uses Serial for debug and Serial1 for the module.
*/
